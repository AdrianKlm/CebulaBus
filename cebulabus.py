...
def send_email(busname, newDate):
    msg = EmailMessage()
    msg.set_content('Na stronie https://'+busname +
                    '.pl pojawiły się nowe bilety dostępne na dzień: ' + newDate)
...
def print_neobus():
    ...
    while do:
        ...
        data = requests.post("https://neobus.pl/", data="ajax=true&dataType=json&module=neotickets&step=1&ticket_type=student&initial_stop=75&final_stop=76&passengers=1&date_there=" +
                             search_format+"&date_return=&initial_stop_name=WARSZAWA+Dw.+Zach.+st.7&final_stop_name=RZESZ%C3%93W+D.A.+PKS+", headers=headers)
        ...
        if not (data.text.count(format)) > 1:
            ...
            write_log("NEOBUS: bilety dostępne do dnia: " +
                      max_date.strftime("%d-%m-%Y"))


def print_flixbus():
    vgm_url = 'https://search.k8s.mfb.io/api/v4/search?from_city_id=40e19c59-8646-11e6-9066-549f350fcb0c&to_city_id=40e279fe-8646-11e6-9066-549f350fcb0c&departure_date=' + \
        departure_date+'&products=%7B%22adult%22%3A1%2C%22bike_slot%22%3A0%7D&currency=PLN&locale=pl&search_by=cities&include_after_midnight_rides=1'
    data = requests.get(vgm_url).text
    json_object = json.loads(str(data))
    dat = json_object["trips"]
    max_date = dat["trips"]
    max_date = dat["results"]
...
sched.add_job(job, 'interval', minutes=int(
    cfg.get('Default', 'RefreshMinutes')))
sched.start()
